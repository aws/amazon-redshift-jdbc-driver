...
public class RedshiftDataSourceFactory implements DataSourceFactory {
  ...
  public java.sql.Driver createDriver(Properties props) throws SQLException {
    if (props != null && !props.isEmpty()) {
      throw new RedshiftException(GT.tr("Unsupported properties: {0}", props.stringPropertyNames()),
          RedshiftState.INVALID_PARAMETER_VALUE);
    }
    return new com.amazon.redshift.Driver();
  }
  ...
  public XADataSource createXADataSource(Properties props) throws SQLException {
    props = new SingleUseProperties(props);
    RedshiftXADataSource dataSource = new RedshiftXADataSource();
    configureBaseDataSource(dataSource, props);
    return dataSource;
  }
}
