 ...
 * status update work only when use {@link RedshiftReplicationStream#read()} method. It means that
...
public interface RedshiftReplicationStream
   ...
   * RedshiftReplicationStream#read()}. If message from backend absent return null. It allow periodically
  ...
  /**
   * <p>Parameter updates by execute {@link RedshiftReplicationStream#read()} method.</p>
   *
   * <p>It is safe to call this method in a thread different than the main thread. However, usually this
   * method is called in the main thread after a successful {@link RedshiftReplicationStream#read()} or
   * {@link RedshiftReplicationStream#readPending()}, to get the LSN corresponding to the received record.</p>
   *
   * @return NOT NULL LSN position that was receive last time via {@link RedshiftReplicationStream#read()}
   *     method
   */
   ...
   * RedshiftReplicationStream#setFlushedLSN(LogSequenceNumber)}</p>
   *
   ...
   * RedshiftReplicationStream#setAppliedLSN(LogSequenceNumber)}</p>
   *
   * <p>It is safe to call this method in a thread different than the main thread.</p>
   *
   ...
   * @see RedshiftReplicationStream#forceUpdateStatus()
   */
   ...
   * explicitly because {@link RedshiftReplicationStream} sends the status to backend periodically by
   ...
   *
   * <p><b>Note:</b> This method can spend much time for logical replication stream on Redshift
   * version 9.6 and lower, because Redshift have bug - during decode big transaction to logical
   * form and during wait new changes Redshift ignore messages from client. As workaround you can
