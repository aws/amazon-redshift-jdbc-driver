...
class RedshiftCallableStatement extends RedshiftPreparedStatement implements CallableStatement {
  ...
  public Object getObject(int i, Map<String, Class<?>> map) throws SQLException {
    if (RedshiftLogger.isEnable()) 
    	connection.getLogger().logFunction(true, i, map);
  ...
  public boolean wasNull() throws SQLException {
    ...
    if (lastIndex == 0) {
      throw new RedshiftException(GT.tr("wasNull cannot be call before fetching a result."),
  ...
  protected void checkIndex(int parameterIndex, int type1, int type2, String getName)
    ...
    if (type1 != this.testReturn[parameterIndex - 1]
      ...
      throw new RedshiftException(
          ...
          RedshiftState.MOST_SPECIFIC_TYPE_DOES_NOT_MATCH);
    }
  }
  ...
  public <T> T getObject(int parameterIndex, Class<T> type) throws SQLException {
    if (RedshiftLogger.isEnable()) 
    	connection.getLogger().logFunction(true, parameterIndex, type);
  	
    if (type == ResultSet.class) {
      return type.cast(getObject(parameterIndex));
    }
    throw new RedshiftException(GT.tr("Unsupported type conversion to {1}.", type),
