...
/*    if (connection.haveMinimumServerVersion(ServerVersion.v11)) {
  ...
  public ResultSet getProcedureColumns(String catalog, String schemaPattern,
    ...
    procedureColQuery.append(" SELECT DISTINCT current_database() AS PROCEDURE_CAT, "
            ...
            + "         AND prokind='p' "
            + "        AND proallargtypes[pos] IN (1042, "
            + "                                     1043) THEN PROBIN_BYTES-4 "
            + " END AS COLUMN_BYTES, "            
            + " CASE WHEN proallargtypes is NULL THEN pg_catalog.format_type(proargtypes[pos], NULL)"
            ...
            + " CASE WHEN (proallargtypes IS NOT NULL) AND prokind='p' AND proallargtypes[pos] = 1700 THEN (PROBIN_BYTES-4)/65536 END as NUMERIC_PRECISION, "
            + " CASE WHEN (proallargtypes IS NOT NULL) AND prokind='p' AND proallargtypes[pos] = 1700 THEN (((PROBIN_BYTES::numeric-4)/65536 - (PROBIN_BYTES-4)/65536) *  65536)::INT END as NUMERIC_SCALE, "
            + " p.proname || '_' || p.prooid AS SPECIFIC_NAME "
