 ...
 * Note: this is a rsjdbc-internal class, so it is not supposed to be used outside of the driver.
 */
public final class EscapedFunctions2 {
  ...
  private static void zeroArgumentFunctionCall(StringBuilder buf, String call, String functionName,
      List<? extends CharSequence> parsedArgs) throws RedshiftException {
    if (!parsedArgs.isEmpty()) {
      throw new RedshiftException(GT.tr("{0} function doesn''t take any argument.", functionName),
          RedshiftState.SYNTAX_ERROR);
    }
    buf.append(call);
  }
