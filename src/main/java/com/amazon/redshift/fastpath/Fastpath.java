...
public class Fastpath {
  ...
  /**
   * Send a function call to the Redshift backend.
   *
  ...
  public Object fastpath(String name, boolean resulttype, FastpathArg[] args) throws SQLException {
    connection.getLogger().log(LogLevel.DEBUG, "Fastpath: calling {0}", name);
    return fastpath(getID(name), resulttype, args);
  }

  /**
   * <p>Send a function call to the Redshift backend by name.</p>
   *
   ...
   * backend.</p>
   *
   * <p>For an example of how this works, refer to com.amazon.redshift.largeobject.LargeObject</p>
   *
   * @param name Function name
   * @param args FastpathArguments to pass to fastpath
   * @return null if no data, byte[] otherwise
   * @throws SQLException if name is unknown or if a database-access error occurs.
   * @see com.amazon.redshift.largeobject.LargeObject
   */
   ...
   *
   * <p>Redshift stores the function id's and their corresponding names in the pg_proc table. To
   ...
   *
   * <p>The com.amazon.redshift.largeobject.LargeObject class performs a query upon it's startup, and passes
  ...
  public int getID(String name) throws SQLException {
    ...
    if (id == null) {
      throw new RedshiftException(GT.tr("The fastpath function {0} is unknown.", name),
